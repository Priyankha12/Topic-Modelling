...
```{r}
# YOUR CODE HERE
posterior_probs <- apply(exp(LDA_model@beta), 2, max)
top100 <- sort(posterior_probs, decreasing=T, index.return = T)
top100vals <- top100$x[1:100]
...
```{r}
# YOUR CODE HERE
plot(1:100, top100vals, "o", xlab = "Word", ylab = "Max probability")
```
...
```{r}
train_docs <- AssociatedPress[1:1500,]
test_docs <- AssociatedPress[-(1:1500),]
LDA_models <- c()
for(k in seq(20,200,20)){
  model <- LDA(train_docs, control=list(iter=20, seed=SEED), k=k, method="Gibbs")
  LDA_models<- c(LDA_models, model)
}
  perplexity_vals <- sapply(LDA_models, function(x) perplexity(x, test_docs))
  plot(perplexity_vals, type="o", col = "blue", axes = F, ann = F)
...
```{r}
# YOUR CODE HERE
train_docs <- yelp_matrix[1:1000,]
test_docs <- yelp_matrix[-(1:1000),]
LDA_models <- c()
for(k in seq(10,100,10)){
  model <- LDA(train_docs, control=list(iter=100, seed=SEED), k=k, method="Gibbs")
  LDA_models<- c(LDA_models, model)
}
top_words = 15
NPMI_vals <- sapply(LDA_models, function(LDA_model) compute_c(LDA_model@beta, test_docs, top_words,"NPMI"))
NPMI_vals <- data.frame(seq(10,100,10), NPMI_vals)
plot(NPMI_vals, type="o", col="blue", xlab="Number of topics", ylab="NPMI", main="Extrinsic topic coherence")
...
```{r}
# YOUR CODE HERE
LCP_vals <- sapply(LDA_models, function(LDA_model) compute_c(LDA_model@beta, train_docs, top_words, "LCP"))
LCP_vals <- data.frame(seq(10,100,10), LCP_vals)
plot(LCP_vals, type="o", col="blue", xlab="Number of topics", ylab="LCP", main="Intrinsic topic coherence")
```
...
```{r}
# YOUR CODE HERE
yelp_train <- yelp_matrix[1:10000,]
LDA_models <- c()
for(k in seq(10,100,10)){
  model <- LDA(yelp_train, control=list(iter=10, seed=SEED), k=k, method="Gibbs")
  LDA_models <- c(LDA_models, model)
}
```
...
clusters = list()
for(i in 1:10) {
clusters[[i]] <- argmax(LDA_models[[i]]@gamma)
}
...
purity = function(assignment, labels) {  
  c <- list()
  for(j in unique(labels)) {
    c[[j]] <- which(labels==j)
  }
  s<-0
  for(i in unique(assignment)){
    omega <- which(assignment==i)
    max_ <- max(sapply(c, matching, b=omega))
    s<- s + max_
  }
...
```{r}
# YOUR CODE HERE
purities <- sapply(clusters, purity, labels=labels)
purity = data.frame(seq(10,100,10), purities)
plot(purity, type="o", col="blue", xlab="Number of Topics", ylab="Purity", main="Document Clustering")
```
